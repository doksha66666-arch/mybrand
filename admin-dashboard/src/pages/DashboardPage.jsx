...
export default function DashboardPage(){
  ...
  useEffect(()=>{let live=true;(async()=>{try{
    const [summary,customers,orderList]=await Promise.allSettled([api.get('/orders/stats/summary'),api.get('/customers'),api.get('/orders')]); if(!live)return;
    const s=summary.status==='fulfilled'?summary.value.data||{}:{}; const c=customers.status==='fulfilled'?customers.value.data||{}:{}; const o=orderList.status==='fulfilled'?orderList.value.data||{}:{};
    ...
    const rows=o.orders||o.data||[]; if(Array.isArray(rows))setOrders(rows.slice(0,5).map(x=>({id:x.orderNumber||x._id||'',customer:x.customer?.name||x.customerName||'—',merchant:x.items?.find(i=>i.merchantNameSnapshot)?.merchantNameSnapshot||'—',amount:x.total||0,status:x.status||''})));
