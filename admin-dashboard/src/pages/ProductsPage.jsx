...
export default function ProductsPage() {
  ...
  return <div dir="rtl" style={styles.page}>
    ...
    <section style={styles.toolbar}>
      ...
      {(search || status !== 'all') && <button type="button" style={styles.secondary} onClick={clearFilters}>مسح الفلاتر</button>}
      <span style={styles.count}>{pagination.total.toLocaleString('ar-EG')} منتج</span>
    </section>
    {loading ? <div style={styles.empty}>جارٍ تحميل المنتجات...</div> : !products.length ? <div style={styles.empty}><strong>{search || status !== 'all' ? 'لا توجد منتجات مطابقة.' : 'لا توجد منتجات بعد.'}</strong>{allowCreate && <button type="button" style={styles.secondary} onClick={() => navigate('/products/add')}>{search || status !== 'all' ? 'إضافة منتج' : 'إضافة أول منتج'}</button>}</div> :
      <>
        ...
        <footer style={styles.paginationBar}>
          <button type="button" style={hasPrevious ? styles.secondary : styles.disabled} disabled={!hasPrevious} onClick={() => setPage(p => Math.max(1, p - 1))}>السابق</button>
          <span style={styles.pageInfo}>صفحة {Math.min(page, Math.max(1, pagination.pages))} من {Math.max(1, pagination.pages)}</span>
          <button type="button" style={hasNext ? styles.secondary : styles.disabled} disabled={!hasNext} onClick={() => setPage(p => p + 1)}>التالي</button>
        </footer>
      </>}
  </div>;
}
