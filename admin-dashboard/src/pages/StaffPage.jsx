...
export default function StaffPage(){
  ...
  const save=async()=>{
    if(!form.name||!form.email)return setStatus('اكتب اسم عضو الفريق والبريد الإلكتروني');
    setSaving(true);setStatus('');
    try{await api.post('/staff',{...form,permissions});setStatus('تمت إضافة عضو الفريق بنجاح');setForm({name:'',email:'',phone:'',role:'viewer'});const {data}=await api.get('/staff');setStaff(data.staff||[])}
  ...
  return <div className="staff-page" dir="rtl"><style>{css}</style>
    <div className="head"><div><div className="crumb">الفريق والشركاء <b>الفريق والصلاحيات</b></div><h1>إضافة عضو فريق وإدارة الصلاحيات</h1><p>أنشئ سجلًا إداريًا لأعضاء الفريق وحدد الدور والصلاحيات لكل قسم.</p></div></div>
    <div className="info-banner"><span>ⓘ</span><div><b>ملاحظة إدارية</b><small>هذه الصفحة تدير سجلات الفريق والصلاحيات الداخلية. لا تنشئ حساب دخول مستقلًا بعد؛ تسجيل الدخول الحالي مخصص لحسابات المشرف.</small></div></div>
    <div className="layout-grid">
      <section>
        ...
        <div className="card"><div className="section-title"><h3>مصفوفة الصلاحيات</h3><span>الصلاحيات محفوظة كسجل إداري وتُراجع قبل ربطها بمداخل وصول مستقلة.</span></div><div className="matrix"><div className="matrix-head"><b>القسم</b>{actions.map(a=><b key={a[0]}>{a[1]}</b>)}</div>{modules.map(([m,label])=><div className="matrix-row" key={m}><span>{label}</span>{actions.map(([a])=><label key={a} className="check"><input type="checkbox" checked={!!permissions[m]?.[a]} onChange={e=>setPerm(m,a,e.target.checked)}/><i/></label>)}</div>)}</div></div>
        <div className="actions"><button className="ghost" onClick={()=>setForm({name:'',email:'',phone:'',role:'viewer'})}>إلغاء</button><button className="primary" disabled={saving} onClick={save}>{saving?'جارٍ الحفظ...':'إضافة عضو الفريق'}</button></div>{status&&<div className="notice">{status}</div>}
      </section>
