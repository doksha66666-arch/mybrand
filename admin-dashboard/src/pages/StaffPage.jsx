...
export default function StaffPage(){
  ...
  const [form,setForm]=useState({name:'',email:'',phone:'',password:'',role:'viewer'}); const [permissions,setPermissions]=useState(emptyPermissions());
  ...
  useEffect(()=>{
    ...
    if(form.role==='super_admin') modules.forEach(([m])=>actions.forEach(([a])=>next[m][a]=true));
      ...
      const map={orders_manager:['orders'],products_manager:['products','categories'],marketing_manager:['marketing','studio'],support:['customers','support'],accountant:['payments','reports'],cashier:['orders','payments']};
  ...
  const reset=()=>{setForm({name:'',email:'',phone:'',password:'',role:'viewer'});setPermissions(emptyPermissions());setStatus('');};
  const save=async()=>{
    if(!form.name||!form.email||!form.password)return setStatus('الاسم والبريد الإلكتروني وكلمة المرور مطلوبة');
    if(form.password.length<12)return setStatus('كلمة المرور يجب أن تكون 12 حرفًا على الأقل');
    setSaving(true);setStatus('');
    try{await api.post('/staff',{...form,permissions});setStatus('تم إنشاء حساب الموظف وتطبيق دوره وصلاحياته بنجاح');reset();await refresh();}
  ...
  return <div className="staff-page" dir="rtl"><style>{css}</style>
    <div className="head"><div><div className="crumb">الفريق والشركاء <b>الفريق والصلاحيات</b></div><h1>إدارة فريق MYBRAND وصلاحياته</h1><p>كل عضو جديد يحصل على حساب دخول مستقل ودور واضح، ويُتحقق من صلاحياته على مستوى الـAPI.</p></div></div>
    <div className="info-banner"><span>✓</span><div><b>نظام وصول فعلي</b><small>حسابات الفريق تستخدم تسجيل الدخول نفسه للوحة الإدارة، بينما يحدد الدور ما يمكن للموظف مشاهدته أو تعديله. حساب المشرف الرئيسي يحتفظ بصلاحية كاملة.</small></div></div>
    <div className="layout-grid">
      <section>
        <div className="card"><h3>بيانات عضو الفريق</h3><div className="fields"><label>الاسم<input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="مثال: أحمد محمد"/></label><label>البريد الإلكتروني<input type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} placeholder="name@mybrand.com"/></label><label>كلمة مرور الدخول<input type="password" value={form.password} onChange={e=>setForm({...form,password:e.target.value})} minLength={12} autoComplete="new-password" placeholder="12 حرفًا على الأقل"/></label><label>رقم التواصل <span>(اختياري)</span><input value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} placeholder="01xxxxxxxxx"/></label></div></div>
