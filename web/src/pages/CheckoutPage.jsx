  ...
  return (
    <form className="checkout-page" onSubmit={submitOrder} dir="rtl">
      <div className="checkout-app">
        ...
        <section className="block address-block" style={getStyle('address')}><div className="addr-row"><div className="addr-body"><div className="addr-name-row"><b>{name || 'بيانات العميل'}</b><span className="addr-tag">{selectedAddress?.label || 'عنوان الشحن'}</span></div><div className="addr-phone">{phone || '01xxxxxxxxx'}</div><div className="addr-text">{[street, building, city, governorate, center, country, postalCode].filter(Boolean).join(' — ') || 'أدخل عنوان التوصيل'}</div></div></div>{savedAddresses.length > 0 && <div className="saved-address-row"><label>استخدم عنوانًا محفوظًا<select value={selectedAddressId} onChange={(e) => { const id = e.target.value; setSelectedAddressId(id); const address = savedAddresses.find((item) => String(item?._id || '') === String(id)); if (!address) return; setName(address.fullName || ''); setPhone(address.phone || ''); setCountry(address.country || 'مصر'); setGovernorate(address.governorate || ''); setCenter(address.center || ''); setCity(address.city || ''); setStreet(address.street || ''); setBuilding(address.building || ''); setPostalCode(address.postalCode || ''); setAddressNotes(address.notes || ''); }}><option value="">اختيار عنوان محفوظ</option>{savedAddresses.map((address) => <option key={address._id} value={address._id}>{address.label || 'عنوان'} — {address.city || 'مدينة غير محددة'}{address.isDefault ? ' (افتراضي)' : ''}</option>)}</select></label></div>}<div className="address-fields"><label>الاسم الكامل<input value={name} onChange={(e) => setName(e.target.value)} placeholder="الاسم الكامل" autoComplete="name" /></label><label>رقم الهاتف<input dir="ltr" inputMode="tel" style={{textAlign:"left"}} value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="01xxxxxxxxx" autoComplete="tel" /></label><label>الدولة<input value={country} onChange={(e) => setCountry(e.target.value)} placeholder="الدولة" autoComplete="country-name" /></label><label>المحافظة<input value={governorate} onChange={(e) => setGovernorate(e.target.value)} placeholder="المحافظة" autoComplete="address-level1" /></label><label>المركز / المنطقة<input value={center} onChange={(e) => setCenter(e.target.value)} placeholder="المركز / المنطقة" /></label><label>المدينة<input value={city} onChange={(e) => setCity(e.target.value)} placeholder="المدينة" autoComplete="address-level2" /></label><label>العنوان بالتفصيل<input value={street} onChange={(e) => setStreet(e.target.value)} placeholder="الشارع" autoComplete="address-line1" /></label><label>رقم المبنى / الدور<input value={building} onChange={(e) => setBuilding(e.target.value)} placeholder="رقم المنزل، الدور، الشقة" autoComplete="address-line2" /></label><label>الرمز البريدي<input dir="ltr" value={postalCode} onChange={(e) => setPostalCode(e.target.value)} placeholder="الرمز البريدي" inputMode="numeric" /></label><label>ملاحظات التوصيل<input value={addressNotes} onChange={(e) => setAddressNotes(e.target.value)} placeholder="علامة مميزة أو ملاحظة إضافية" /></label></div></section>
