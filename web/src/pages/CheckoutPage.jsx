...
export default function CheckoutPage() {
  ...
  const updateAddressField = (setter) => (event) => { setSelectedAddressId(''); setter(event.target.value); };
  useEffect(() => {
    let active = true;
    ...
    if (user) {
      ...
      api.get('/account').then(({ data }) => {
        ...
        if (preferred && mounted) {
          ...
          setGovernorate(preferred.governorate || data?.user?.governorate || '');
          setCenter(preferred.center || data?.user?.center || '');
          setCity(preferred.city || '');
          setStreet(preferred.street || '');
          setBuilding(preferred.building || '');
          setPostalCode(preferred.postalCode || '');
          setAddressNotes(preferred.notes || '');
        }
      }).catch(() => {});
    }
    return () => { mounted = false; };
  ...
  const submitOrder = async (e) => {
    ...
    try {
      const { data } = await api.post('/orders', {
        ...
        shippingAddress: { country: country.trim(), governorate: governorate.trim(), center: center.trim(), city: city.trim(), street: street.trim(), building: building.trim(), postalCode: postalCode.trim(), notes: addressNotes.trim() },
        paymentMethod,
  ...
  return (
    <form className="checkout-page" onSubmit={submitOrder} dir="rtl">
      <div className="checkout-app">
        ...
        <section className="block address-block" style={getStyle('address')}><div className="addr-row"><div className="addr-body"><div className="addr-name-row"><b>{name || 'بيانات العميل'}</b><span className="addr-tag">{selectedAddress?.label || 'عنوان الشحن'}</span></div><div className="addr-phone">{phone || '01xxxxxxxxx'}</div><div className="addr-text">{[street, building, city, governorate, center, country, postalCode].filter(Boolean).join(' — ') || 'أدخل عنوان التوصيل'}</div></div></div>{savedAddresses.length > 0 && <div className="saved-address-row"><label>استخدم عنوانًا محفوظًا<select value={selectedAddressId} onChange={(e) => { const id = e.target.value; setSelectedAddressId(id); const address = savedAddresses.find((item) => String(item?._id || '') === String(id)); if (!address) return; setName(address.fullName || ''); setPhone(address.phone || ''); setCountry(address.country || 'مصر'); setGovernorate(address.governorate || ''); setCenter(address.center || ''); setCity(address.city || ''); setStreet(address.street || ''); setBuilding(address.building || ''); setPostalCode(address.postalCode || ''); setAddressNotes(address.notes || ''); }}><option value="">اختيار عنوان محفوظ</option>{savedAddresses.map((address) => <option key={address._id} value={address._id}>{address.label || 'عنوان'} — {address.city || 'مدينة غير محددة'}{address.isDefault ? ' (افتراضي)' : ''}</option>)}</select></label></div>}<div className="address-fields"><label>الاسم الكامل<input value={name} onChange={updateAddressField(setName)} placeholder="الاسم الكامل" autoComplete="name" /></label><label>رقم الهاتف<input dir="ltr" inputMode="tel" style={{textAlign:"left"}} value={phone} onChange={updateAddressField(setPhone)} placeholder="01xxxxxxxxx" autoComplete="tel" /></label><label>الدولة<input value={country} onChange={updateAddressField(setCountry)} placeholder="الدولة" autoComplete="country-name" /></label><label>المحافظة<input value={governorate} onChange={updateAddressField(setGovernorate)} placeholder="المحافظة" autoComplete="address-level1" /></label><label>المركز / المنطقة<input value={center} onChange={updateAddressField(setCenter)} placeholder="المركز / المنطقة" /></label><label>المدينة<input value={city} onChange={updateAddressField(setCity)} placeholder="المدينة" autoComplete="address-level2" /></label><label>العنوان بالتفصيل<input value={street} onChange={updateAddressField(setStreet)} placeholder="الشارع" autoComplete="address-line1" /></label><label>رقم المبنى / الدور<input value={building} onChange={updateAddressField(setBuilding)} placeholder="رقم المنزل، الدور، الشقة" autoComplete="address-line2" /></label><label>الرمز البريدي<input dir="ltr" value={postalCode} onChange={updateAddressField(setPostalCode)} placeholder="الرمز البريدي" inputMode="numeric" /></label><label>ملاحظات التوصيل<input value={addressNotes} onChange={updateAddressField(setAddressNotes)} placeholder="علامة مميزة أو ملاحظة إضافية" /></label></div></section>
