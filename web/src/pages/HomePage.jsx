...
export default function HomePage() {
  ...
  const merchantQuery = merchantId ? `?merchant=${encodeURIComponent(merchantId)}` : '';
  useEffect(() => { let active = true; api.get('/homepage', { params: merchantId ? { merchant: merchantId } : undefined }).then(({ data: result }) => active && setData(result)).catch(() => active && setData({})).finally(() => active && setLoading(false)); return () => { active = false; }; }, [merchantId]);
  useEffect(() => { const q = searchParams.get('q') || ''; setSearchInput(q); if (!q) { setSearchResults([]); setSearched(false); return; } let active = true; setSearching(true); api.get('/products', { params: merchantId ? { search: q, merchant: merchantId } : { search: q } }).then(({ data: result }) => active && setSearchResults(result.products || [])).catch(() => active && setSearchResults([])).finally(() => active && setSearching(false)); return () => { active = false; }; }, [searchParams]);
  ...
  return <main className="home-reference" dir="rtl"><div className="app"><header><div className="head-row"><Link to="/" className="logo">MYBRAND</Link><form className="search-pill" onSubmit={submitSearch}><Icon type="search"/><input value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="فستان صيفي، حذاء رياضي..." aria-label="البحث" autoComplete="off"/><button type="submit" aria-label="بحث">بحث</button></form><Link to="/cart" className="head-icon" aria-label="السلة"><Icon type="cart"/></Link></div></header>
    ...
    <div className="home-layout-sections">
      ...
      {section('offers', <><section className="flash"><div className="flash-left"><span className="t1">⚡ فلاش سيل حتى ٧٠٪</span><span className="t2">العرض لفترة محدودة</span></div><Countdown/></section><div className="tile-row"><Link to={`/new-arrivals${merchantQuery}`} className="tile tile-dark"><h4>وصل حديثًا</h4><span>تشكيلة الأسبوع</span></Link><Link to={`/offers${merchantQuery}`} className="tile tile-sale"><h4>عروض وخصومات</h4><span>قطع مختارة</span></Link></div></>)}
      {section('categories', <CategoryImages categories={categories}/>)}
      {section('best', <><div className="sec-title"><h2>🔥 الأكثر مبيعًا</h2><Link className="more" to={`/categories${merchantQuery}`}>عرض الكل</Link></div>{loading ? <div className="grid">{Array.from({ length: 6 }).map((_, i) => <div className="card" key={i}><div className="card-img"/><div className="card-info"><div className="card-title">جاري تحميل المنتج...</div></div></div>)}</div> : products.length ? <div className="grid">{products.slice(0, 8).map((p) => <ProductCard key={p._id || p.id} product={p} merchantId={merchantId}/>)}</div> : <div className="category-empty">لا توجد منتجات منشورة حاليًا. أضف المنتجات من لوحة الإدارة.</div>}</>)}
      {section('featured', discounted.length > 0 ? <><div className="sec-title"><h2>⚡ عروض اليوم</h2><Link className="more" to={`/offers${merchantQuery}`}>عرض الكل</Link></div><div className="grid">{discounted.slice(0, 4).map((p) => <ProductCard key={`d-${p._id || p.id}`} product={p} merchantId={merchantId}/>)}</div></> : null)}
