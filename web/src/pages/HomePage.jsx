...
function ProductCard({ product }) {
  ...
  return <article className="card"><div className="card-img" style={{ background: product.bg || '#F3F4F6' }}>{(discount > 0 || old > price) && <span className="off-tag">-{Math.round(discount || ((old - price) / old * 100))}٪</span>}<Link to={`/products/${product.slug || id}`} className="card-img-link" aria-label={`فتح ${title}`}><ImageWithFallback src={image} alt={title} /></Link><button type="button" className={`fav-ic ${liked ? 'liked' : ''}`} onClick={() => toggleWishlist(id)} aria-label={liked ? 'إزالة من المفضلة' : 'إضافة للمفضلة'}>♥</button></div><div className="card-info"><Link to={`/products/${product.slug || id}`} className="card-title">{title}</Link><div className="price-line"><span className="price-now">{price.toLocaleString('ar-EG')}ج</span>{old > price && <span className="price-old">{old.toLocaleString('ar-EG')}ج</span>}</div><div className="rating-line">★ {Number(product.rating || 0).toFixed(1)} ({Number(product.reviewsCount || product.reviewCount || 0).toLocaleString('ar-EG')} تقييم)</div><button type="button" className="home-add-cart" disabled={outOfStock} onClick={add}>{outOfStock ? 'نفد المخزون' : busy ? 'تمت الإضافة ✓' : hasOptions ? 'اختر الخيارات' : 'أضف للسلة'}</button></div></article>;
}
