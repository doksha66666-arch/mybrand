...
export default function LoginPage() {
  ...
  const submit = async (e) => { e.preventDefault(); setError(''); setSubmitting(true); try { await login(email, password); try { localStorage.removeItem('mybrand_reset_return'); localStorage.removeItem('mybrand_social_return'); } catch (_) {} navigate(returnTo, { replace: true, state: returnState }); } catch (err) { if (err?.response?.data?.needsVerification) { navigate(`/verify-email?email=${encodeURIComponent(err.response.data.email || email)}`, { state: { returnTo, checkoutState: returnState } }); return; } setError(err?.response?.data?.message || 'تعذر تسجيل الدخول'); } finally { setSubmitting(false); } };
  ...
  return <div className="app">
    ...
    <form className="form" style={getStyle('form')} onSubmit={submit}>
      ...
      <div className="forgot-row"><Link to="/forgot-password" state={returnTo !== '/' ? { returnTo, checkoutState: returnState } : undefined}>نسيت كلمة المرور؟</Link></div><button className="primary-btn gradient" type="submit" disabled={submitting}>{submitting ? 'جارٍ تسجيل الدخول...' : 'تسجيل الدخول'}</button>{error && <div className="login-error" role="alert">{error}</div>}
    </form>
