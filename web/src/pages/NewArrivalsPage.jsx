...
export default function NewArrivalsPage() {
  ...
  return <main className="new-arrivals-page" dir="rtl">
    ...
    {error ? <div className="new-arrivals-empty">{error}</div> : loading ? <div className="new-arrivals-grid">{Array.from({length:8}).map((_,i)=><div className="new-arrivals-skeleton" key={i}/>)}</div> : products.length ? <div className="new-arrivals-grid">{products.map(product => <ProductCard key={product._id || product.id || product.slug} product={product} merchantId={merchantId}/>)}</div> : <div className="new-arrivals-empty"><strong>لا توجد منتجات جديدة حاليًا</strong><span>ستظهر هنا تلقائيًا عند نشر وقبول منتجات جديدة.</span><Link to="/">العودة للرئيسية</Link></div>}
  </main>;
}
