...
const readResetReturn = () => {
  if (typeof window === 'undefined') return null;
  try {
    const raw = localStorage.getItem('mybrand_reset_return');
    const parsed = raw ? JSON.parse(raw) : null;
    if (!parsed?.returnTo || !String(parsed.returnTo).startsWith('/')) return null;
    return {
      returnTo: parsed.returnTo,
      checkoutState: parsed.checkoutState || undefined,
    };
  } catch {
    return null;
  }
...
export default function ResetPasswordPage() {
  ...
  const submit = async (e) => {
    ...
    if (!token) { setLoading(false); return setError('رابط إعادة التعيين غير صالح.'); }
    if (password.length < 12) { setLoading(false); return setError('كلمة المرور يجب أن تكون 12 حرفًا على الأقل.'); }
    if (password !== confirm) { setLoading(false); return setError('كلمتا المرور غير متطابقتين.'); }
    try {
      const { data } = await api.post('/auth/reset-password', { token, password });
      setMessage(data.message);
      const resetReturn = readResetReturn();
      try { localStorage.removeItem('mybrand_reset_return'); } catch (_) {}
      setTimeout(() => {
        navigate('/login', resetReturn?.returnTo ? { state: resetReturn } : undefined);
      }, 1200);
