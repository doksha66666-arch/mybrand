...
export default function TrackingPage() {
  ...
  return <div className="tracking-app" dir="rtl">
    <div className="tracking-topbar" style={getStyle('topbar')}><button className="tracking-back-btn" type="button" onClick={() => navigate('/orders')}>‹</button><h1>تتبع الطلب</h1><button className="tracking-support-btn" type="button" onClick={() => navigate('/contact')}>💬</button></div>
    ...
    <div className="tracking-block" style={getStyle('items')}><div className="tracking-block-title">محتويات الطلب ({items.length})</div>{items.length ? items.map((item, index) => { const attrs = optionEntries(item); const product = asObject(item.product); const image = item.imageSnapshot || item.image || (Array.isArray(product.images) ? product.images[0] : ''); const title = item.nameSnapshot || item.nameAr || item.name || product.nameAr || product.name || 'منتج'; const unitPrice = item.unitPrice ?? item.price ?? 0; return <div className="order-item" key={item._id || `${title}-${index}`}><div className="order-img">{image ? <img src={image} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}} /> : null}</div><div className="order-body"><div className="order-title">{title}</div><div className="order-attrs">{attrs.map(([name, value]) => <span key={`${name}-${value}`} style={{display:'block'}}>{name}: {value}</span>)}{item.productCodeSnapshot&&<span>كود المنتج: {item.productCodeSnapshot}</span>}<span>الكمية: {Number(item.quantity || 0)}</span></div><div className="order-price">{Number(unitPrice || 0).toLocaleString('ar-EG')} ج.م</div></div></div>; }) : <div className="tracking-block-title">لا توجد تفاصيل منتجات متاحة.</div>}</div>
    <div className="action-row" style={getStyle('actions')}>{canCancel && <button className="action-btn" type="button" onClick={cancelOrder} disabled={cancelling}>{cancelling ? 'جارٍ الإلغاء...' : '✕ إلغاء الطلب'}</button>}<button className="action-btn" type="button" onClick={() => navigate('/contact')}>💬 تواصل مع الدعم</button><button className="action-btn primary" type="button" onClick={() => navigate('/orders')}>العودة إلى طلباتي</button></div>
  </div>;
}
